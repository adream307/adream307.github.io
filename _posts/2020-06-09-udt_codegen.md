...
本篇文章是这个系列的最后一篇文章，我们介绍如何在 `UDT` 上做 `Codegen`，和[前文](../udt_udf_example/)类似， `Codegen` 实现的自定义函数要求如下:
- 该函数接受两个 `my_point` 类型的参数作为输入
- 参数名记为 `x，y`
- 函数输出 `my_point(x.x+y.x, x.y+y.y)`

## Codegen
和[前文](../codegen_example/)类似， `Codegen` 实现的自定义函数也是从 `Expression` 继承，并实现 `doGenCode` 方法。`doGenCode` 也按照 `nullable` 的值分为两部分，完整核心代码如下：
```scala
...
```
和[前文](../codegen_example/) `SparkSQL` 内置数据类型的 `Codegen` 略有不同，在 `UDT` 的 `Codegen` 中, 最后赋值给变量 `${ev.value}` 的是 `my_point` 的序列化，而不是 `my_point`。
